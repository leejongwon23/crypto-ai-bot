...
BASE_URL="https://api.bybit.com"; BINANCE_BASE_URL="https://fapi.binance.com"
...
try:
    ...
    STRATEGY_CONFIG={"단기":{"interval":"240","limit":1000,"binance_interval":"4h"},
                     "중기":{"interval":"D","limit":500,"binance_interval":"1d"},
                     "장기":{"interval":"D","limit":500,"binance_interval":"1d"}}
    def get_REGIME():
        return {"enabled":False,"atr_window":14,"rsi_window":14,"trend_window":50,"vol_high_pct":0.9,"vol_low_pct":0.5}

# ========================= 공통 유틸 =========================
def _parse_ts_series(s:pd.Series)->pd.Series:
    """혼합 timestamp를 UTC→KST로 안전 변환"""
    if s is None: return pd.to_datetime(pd.Series([],dtype=object),errors="coerce",utc=True)
    ...
    if is_numeric.any():
        ...
        unit_ms=(s_str.where(is_numeric).str.len()>=13).any()
        ts_num=pd.to_datetime(num,unit="ms" if unit_ms else "s",errors="coerce",utc=True)
    else:
        ts_num=pd.Series(pd.NaT,index=s.index,dtype="datetime64[ns, UTC]")
    iso_ms=pd.to_datetime(s_str,format="%Y-%m-%dT%H:%M:%S.%fZ",errors="coerce",utc=True)
    iso_s =pd.to_datetime(s_str.where(iso_ms.isna()),format="%Y-%m-%dT%H:%M:%SZ",errors="coerce",utc=True)
...
def _discover_from_models():
    ...
    for fn in os.listdir(md):
        if not (fn.endswith(".pt") or fn.endswith(".meta.json")): continue
...
def _select_60(symbols): 
    if len(symbols)>=60: return symbols[:60]
    need=60-len(symbols); filler=[s for s in _BASELINE_SYMBOLS if s not in symbols][:need]
...
# ========================= 그룹 순서 제어(지속성) =========================
_STATE_DIR="/persistent/state"; _STATE_PATH=os.path.join(_STATE_DIR,"group_order.json")
class GroupOrderManager:
    """현재 그룹 인덱스/학습 완료 심볼 집합 관리 + 파일 지속성"""
    def __init__(self,groups:List[List[str]]):
        self.groups=[list(g) for g in groups]; self.idx=0; self.trained={}; self._load()
    def _load(self):
        try:
            ...
            if os.path.isfile(_STATE_PATH):
                ...
                if saved_groups: self.groups=saved_groups
                self.idx=int(st.get("idx",0))
                self.trained={int(k):set(v) for k,v in st.get("trained",{}).items()}
                print(f"[🧭 그룹상태 로드] idx={self.idx}, trained={{"+", ".join(f"{k}:{list(v)}" for k,v in self.trained.items())+"}}")
        except Exception as e: print(f"[⚠️ 그룹상태 로드 실패] {e}")
    def _save(self):
        try:
            os.makedirs(_STATE_DIR,exist_ok=True)
            payload={"groups":self.groups,"idx":self.idx,"trained":{k:list(v) for k,v in self.trained.items()},"symbols":SYMBOLS}
            json.dump(payload,open(_STATE_PATH,"w",encoding="utf-8"),ensure_ascii=False,indent=2)
        except Exception as e: print(f"[⚠️ 그룹상태 저장 실패] {e}")
    def current_index(self)->int: return max(0,min(self.idx,max(0,len(self.groups)-1)))
    ...
    def ready_for_group_predict(self)->bool:
        ...
        if ready: print(f"[🚦 예측대기] 그룹{i} 전체 학습 완료 → 예측 실행 준비")
        return ready
    def mark_group_predicted(self):
        i=self.current_index(); print(f"[✅ 예측완료] 그룹{i} → 다음 그룹으로 이동")
        self.idx=(i+1)%max(1,len(self.groups)); self.trained.setdefault(self.idx,set()); self._save()
    def reset(self,start_index:int=0):
        self.idx=max(0,min(start_index,max(0,len(self.groups)-1))); self.trained={self.idx:set()}; self._save(); print(f"[♻️ 그룹순서 리셋] idx={self.idx}")
    def rebuild_groups(self,symbols:Optional[List[str]]=None,group_size:int=5):
        self.groups=_compute_groups(symbols or SYMBOLS,group_size); self.reset(0); print(f"[🧱 그룹재구성] 총 {len(symbols or SYMBOLS)}개 → {len(self.groups)}그룹")
    ...
    def get(cls,key,ttl_sec=None):
        ...
        if key in cls._cache:
            if ttl_sec is None or now-cls._ttl.get(key,0)<ttl_sec:
                print(f"[캐시 HIT] {key}"); return cls._cache[key]
            else: print(f"[캐시 EXPIRED] {key}"); cls.delete(key)
        return None
    @classmethod
    def set(cls,key,value): cls._cache[key]=value; cls._ttl[key]=time.time(); print(f"[캐시 SET] {key}")
    @classmethod
    def delete(cls,key):
        if key in cls._cache: del cls._cache[key]; cls._ttl.pop(key,None); print(f"[캐시 DELETE] {key}")
    @classmethod
    def clear(cls): cls._cache.clear(); cls._ttl.clear(); print("[캐시 CLEAR ALL]")

def _binance_blocked_until(): return CacheManager.get("binance_blocked_until")
def _is_binance_blocked(): 
    until=_binance_blocked_until(); return until is not None and time.time()<until
def _block_binance_for(seconds=1800):
    CacheManager.set("binance_blocked_until",time.time()+seconds); print(f"[🚫 Binance 차단] {seconds}초 동안 Binance 폴백 비활성화")

# ========================= 실패 로깅 경량 헬퍼 =========================
def safe_failed_result(symbol,strategy,reason=""):
    try:
        from failure_db import insert_failure_record
        payload={"symbol":symbol or "UNKNOWN","strategy":strategy or "UNKNOWN","model":"utils","reason":reason,
    ...
    except Exception as e: print(f"[⚠️ safe_failed_result 실패] {e}")

# ========================= 기타 유틸 =========================
def get_btc_dominance():
    global BTC_DOMINANCE_CACHE
    now=time.time()
    if now-BTC_DOMINANCE_CACHE["timestamp"]<1800: return BTC_DOMINANCE_CACHE["value"]
    try:
        res=requests.get("https://api.coinpaprika.com/v1/global",timeout=10,headers=REQUEST_HEADERS); res.raise_for_status()
        dom=float(res.json()["bitcoin_dominance_percentage"])/100
        BTC_DOMINANCE_CACHE={"value":round(dom,4),"timestamp":now}; return BTC_DOMINANCE_CACHE["value"]
    except: return BTC_DOMINANCE_CACHE["value"]
...
def future_gains_by_hours(df:pd.DataFrame,horizon_hours:int)->np.ndarray:
    ...
    out=np.zeros(len(df),dtype=np.float32); H=pd.Timedelta(hours=int(horizon_hours)); j0=0
...
def _fix_ohlc_consistency(df:pd.DataFrame)->pd.DataFrame:
    ...
    for c in ["open","high","low","close"]: df.loc[df[c]<=0,c]=np.nan
    df.loc[df["volume"]<0,"volume"]=np.nan
    df[["open","high","low","close","volume"]]=df[["open","high","low","close","volume"]].ffill()
    df=df.dropna(subset=["open","high","low","close","volume"])
    mx=df[["open","close","low"]].max(axis=1); mn=df[["open","close","high"]].min(axis=1)
    df["high"]=np.maximum(df["high"].values,mx.values); df["low"]=np.minimum(df["low"].values,mn.values)
    return df
def _winsorize_prices(df:pd.DataFrame,lower_q=0.001,upper_q=0.999)->pd.DataFrame:
    if df is None or df.empty: return df
    df=df.copy(); cols=["open","high","low","close"]
    for c in cols:
        if c not in df.columns: continue
        s=pd.to_numeric(df[c],errors="coerce"); lo=s.quantile(lower_q); hi=s.quantile(upper_q)
        df[c]=s.clip(lower=lo,upper=hi).astype(np.float32)
    if "volume" in df.columns:
        v=pd.to_numeric(df["volume"],errors="coerce"); lo=max(0.0,v.quantile(lower_q)); hi=v.quantile(upper_q)
...
def create_dataset(features,window=10,strategy="단기",input_size=None):
    ...
    if not isinstance(features,list) or len(features)<=window:
        print(f"[⚠️ 부족] features length={len(features) if isinstance(features,list) else 'Invalid'}, window={window}"); return _dummy(symbol_name)
    try:
        df=_pd.DataFrame(features); df["timestamp"]=_parse_ts_series(df.get("timestamp")); df=df.dropna(subset=["timestamp"]).sort_values("timestamp").reset_index(drop=True)
        ...
        if not feature_cols: print("[❌ feature_cols 없음]"); return _dummy(symbol_name)
        from config import MIN_FEATURES as _MINF
        if len(feature_cols)<_MINF:
            for i in range(len(feature_cols),_MINF): df[f"pad_{i}"]=np.float32(0.0); feature_cols.append(f"pad_{i}")
        scaler=MinMaxScaler(); scaled=scaler.fit_transform(df[feature_cols].astype(np.float32))
        df_s=pd.DataFrame(scaled.astype(np.float32),columns=feature_cols); df_s["timestamp"]=df["timestamp"].values; df_s["high"]=df["high"] if "high" in df.columns else df["close"]
        ...
        for i in range(window,len(features)):
            seq=features[i-window:i]; base=features[i]
...
def _normalize_df(df:pd.DataFrame)->pd.DataFrame:
    ...
    df=df.copy()
    if "timestamp" in df.columns: df["timestamp"]=_parse_ts_series(df["timestamp"])
    elif "time" in df.columns:   df["timestamp"]=_parse_ts_series(df["time"])
...
def _clip_tail(df:pd.DataFrame,limit:int)->pd.DataFrame:
    ...
    try:
        if getattr(ts.dt,"tz",None) is None: ts=ts.dt.tz_localize("UTC").dt.tz_convert("Asia/Seoul")
...
def get_kline(symbol:str,interval:str="60",limit:int=300,max_retry:int=2,end_time=None)->pd.DataFrame:
    real_symbol=SYMBOL_MAP["bybit"].get(symbol,symbol); target_rows=int(limit)
    ...
    while total<target_rows:
        ...
        for attempt in range(max_retry):
            try:
                ...
                res=requests.get(f"{BASE_URL}/v5/market/kline",params=params,timeout=10,headers=REQUEST_HEADERS); res.raise_for_status()
                data=res.json(); raw=(data or {}).get("result",{}).get("list",[])
                if not raw: print(f"[❌ 데이터 없음] {real_symbol} (시도 {attempt+1})"); break
                ...
                else: df_chunk=pd.DataFrame(raw)
                ...
                print(f"[에러] get_kline({real_symbol}) 네트워크 실패 → {e}"); time.sleep(1); continue
            except Exception as e:
                print(f"[에러] get_kline({real_symbol}) 실패 → {e}"); time.sleep(0.5); continue
        if not success: break
    if collected:
        df=_normalize_df(pd.concat(collected,ignore_index=True)); print(f"[📊 수집 완료] {symbol}-{interval} → 총 {len(df)}개 봉 확보"); return df
            ...
            try:
                ...
                try: res.raise_for_status()
                    ...
                    if getattr(he.response,"status_code",None)==418:
                        print("[🚨 Binance 418 감지] 자동 백오프 및 폴백 비활성화"); _block_binance_for(1800)
                        return pd.DataFrame(columns=["timestamp","open","high","low","close","volume","datetime"])
                    raise
                raw=res.json()
                if not raw: print(f"[❌ Binance 데이터 없음] {real_symbol}-{interval} (시도 {attempt+1})"); break
                if isinstance(raw[0],(list,tuple)) and len(raw[0])>=6:
                    df_chunk=pd.DataFrame(raw,columns=["timestamp","open","high","low","close","volume","close_time","quote_asset_volume","trades","taker_base_vol","taker_quote_vol","ignore"])
                else: df_chunk=pd.DataFrame(raw)
                df_chunk=_normalize_df(df_chunk)
                if df_chunk.empty: break
                collected.append(df_chunk); total+=len(df_chunk); success=True
                ...
                print(f"[에러] get_kline_binance({real_symbol}) 네트워크 실패 → {e}"); time.sleep(1); continue
            except Exception as e:
                print(f"[에러] get_kline_binance({real_symbol}) 실패 → {e}"); time.sleep(0.5); continue
        if not success: break
    if collected:
        df=_normalize_df(pd.concat(collected,ignore_index=True)); print(f"[📊 Binance 수집 완료] {symbol}-{interval} → 총 {len(df)}개 봉 확보"); return df
    print(f"[❌ 최종 실패] {symbol}-{interval} → 수집된 봉 없음")
    return pd.DataFrame(columns=["timestamp","open","high","low","close","volume","datetime"])
...
def get_merged_kline_by_strategy(symbol:str,strategy:str)->pd.DataFrame:
    ...
    if not config: print(f"[❌ 실패] 전략 설정 없음: {strategy}"); return pd.DataFrame()
    ...
    def fetch_until_target(fetch_func,src):
        ...
        while cnt<max_total and len(total)<max_rep:
            ...
            oldest=dfc["timestamp"].min(); end=pd.to_datetime(oldest).tz_convert("Asia/Seoul")-pd.Timedelta(milliseconds=1)
        dff=_normalize_df(pd.concat(total,ignore_index=True)) if total else pd.DataFrame()
        print(f"[✅ {src} 수집 완료] {symbol}-{strategy} → {len(dff)}개"); return dff
    ...
    if df_all.empty: print(f"[⏩ 학습 스킵] {symbol}-{strategy} → 거래소 데이터 전무"); return pd.DataFrame()
    ...
    for c in ["timestamp","open","high","low","close","volume"]:
        if c not in df_all.columns: df_all[c]=0.0 if c!="timestamp" else pd.Timestamp.now(tz="Asia/Seoul")
    df_all.attrs["augment_needed"]=len(df_all)<base_limit
    print(f"[🔄 병합 완료] {symbol}-{strategy} → 최종 {len(df_all)}개 (목표 {base_limit}개)")
    if len(df_all)<base_limit: print(f"[⚠️ 경고] {symbol}-{strategy} 데이터 부족 ({len(df_all)}/{base_limit})")
    return df_all

# ========================= 전략별 Kline (캐시 포함) =========================
def get_kline_by_strategy(symbol:str,strategy:str):
    cache_key=f"{symbol}-{strategy}"; cached=CacheManager.get(cache_key,ttl_sec=600)
    if cached is not None: print(f"[✅ 캐시 사용] {symbol}-{strategy} → {len(cached)}개 봉"); return cached
    try:
        cfg=STRATEGY_CONFIG.get(strategy,{"limit":300,"interval":"D"}); limit=int(cfg.get("limit",300)); interval=cfg.get("interval","D")
        ...
        if len(df_bybit)<int(limit*0.9) and BINANCE_ENABLED and not _is_binance_blocked():
            print(f"[📡 Binance 2차 반복 수집 시작] {symbol}-{strategy} (limit={limit})"); end=None
            while total_binance<limit:
                try:
                    ...
                    print(f"[❌ Binance 수집 실패] {symbol}-{strategy} → {be}"); break
        elif len(df_bybit)<int(limit*0.9): print("[⛔ Binance 폴백 스킵] 비활성화 또는 일시차단 상태")
        ...
        if total<min_required: print(f"[🚨 최종 부족] {symbol}-{strategy} → {total}/{min_required} (학습/예측 영향 가능)")
        else: print(f"[✅ 수집 성공] {symbol}-{strategy} → 총 {total}개")
        df.attrs["augment_needed"]=total<limit; df.attrs["enough_for_training"]=total>=min_required
        CacheManager.set(cache_key,df); return df
    except Exception as e:
        print(f"[❌ 데이터 수집 실패] {symbol}-{strategy} → {e}"); safe_failed_result(symbol,strategy,reason=str(e))
        return pd.DataFrame(columns=["timestamp","open","high","low","close","volume","datetime"])
...
def prefetch_symbol_groups(strategy:str):
    for group in SYMBOL_GROUPS:
        for sym in group:
            ...
            except Exception as e: print(f"[⚠️ prefetch 실패] {sym}-{strategy}: {e}")

# ========================= 실시간 티커 =========================
def get_realtime_prices():
    url=f"{BASE_URL}/v5/market/tickers"; params={"category":"linear"}
    try:
        res=requests.get(url,params=params,timeout=10,headers=REQUEST_HEADERS); res.raise_for_status()
        data=res.json()
...
def compute_features(symbol:str,df:pd.DataFrame,strategy:str,required_features:list=None,fallback_input_size:int=None)->pd.DataFrame:
    from config import FEATURE_INPUT_SIZE; import ta
    cache_key=f"{symbol}-{strategy}-features"; cached=CacheManager.get(cache_key,ttl_sec=600)
    if cached is not None: print(f"[캐시 HIT] {cache_key}"); return cached
    if df is None or df.empty or not isinstance(df,pd.DataFrame):
        print(f"[❌ compute_features 실패] 입력 DataFrame empty or invalid"); safe_failed_result(symbol,strategy,reason="입력DataFrame empty"); return pd.DataFrame()
    df=df.copy()
    ...
    if len(df)<20:
        print(f"[⚠️ 피처 실패] {symbol}-{strategy} → row 수 부족: {len(df)}"); safe_failed_result(symbol,strategy,reason=f"row 부족 {len(df)}"); return df
    try:
        df["ma20"]=df["close"].rolling(window=20,min_periods=1).mean()
        delta=df["close"].diff(); gain=delta.where(delta>0,0).rolling(window=14,min_periods=1).mean(); loss=-delta.where(delta<0,0).rolling(window=14,min_periods=1).mean()
        rs=gain/(loss+1e-6); df["rsi"]=100-(100/(1+rs))
        ...
        df["volatility"]=df["high"]-df["low"]; df["trend_score"]=(df["close"]>df["ma20"]).astype(int)
        df["ema50"]=df["close"].ewm(span=50,adjust=False).mean(); df["ema100"]=df["close"].ewm(span=100,adjust=False).mean(); df["ema200"]=df["close"].ewm(span=200,adjust=False).mean()
        ...
        if regime_cfg.get("enabled",False):
            atr_win=int(regime_cfg.get("atr_window",14)); trend_win=int(regime_cfg.get("trend_window",50))
            vol_high=float(regime_cfg.get("vol_high_pct",0.9)); vol_low=float(regime_cfg.get("vol_low_pct",0.5))
            df["atr_val"]=_ta.volatility.average_true_range(df["high"],df["low"],df["close"],window=atr_win,fillna=True)
            thr_high=df["atr_val"].quantile(vol_high); thr_low=df["atr_val"].quantile(vol_low)
            df["vol_regime"]=np.where(df["atr_val"]>=thr_high,2,np.where(df["atr_val"]<=thr_low,0,1))
            df["ma_trend"]=df["close"].rolling(window=trend_win,min_periods=1).mean(); slope=df["ma_trend"].diff()
            df["trend_regime"]=np.where(slope>0,2,np.where(slope<0,0,1)); df["regime_tag"]=df["vol_regime"]*3+df["trend_regime"]
        # 정리/스케일
        ...
        if len(feat_cols)<_FIS:
            for i in range(len(feat_cols),_FIS): df[f"pad_{i}"]=0.0; feat_cols.append(f"pad_{i}")
        df[feat_cols]=_downcast_numeric(df[feat_cols]).astype(np.float32)
        df[feat_cols]=MinMaxScaler().fit_transform(df[feat_cols])
    except Exception as e:
        print(f"[❌ compute_features 실패] feature 계산 예외 → {e}"); safe_failed_result(symbol,strategy,reason=f"feature 계산 실패: {e}"); return df
    if df.empty or df.isnull().values.any():
        print(f"[❌ compute_features 실패] 결과 DataFrame 문제 → 빈 df 또는 NaN 존재"); safe_failed_result(symbol,strategy,reason="최종 결과 DataFrame 오류"); return df
    print(f"[✅ 완료] {symbol}-{strategy}: 피처 {df.shape[0]}개 생성"); print(f"[🔍 feature 상태] {symbol}-{strategy} → shape: {df.shape}, NaN: {df.isnull().values.any()}, 컬럼수: {len(df.columns)}")
    CacheManager.set(cache_key,df); return df
