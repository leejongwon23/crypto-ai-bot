...
def get_kline_by_strategy(symbol: str, strategy: str, end_slack_min: int = 0):
    """
    Bybit → Binance 순으로 데이터를 조회.
    - 캐시 TTL 600초 유지
    - 데이터가 일부만 조회되어도 그대로 반환 (절대 빈 DF 반환 안 함)
    - 최소 행수(0.9*limit) 이하 시 자동 폴백 병합 수행
    """
    try:
        ...
        if df.empty:
            ...
            df = pd.DataFrame({
                ...
                "close": [1.0], "volume": [0.0], "datetime": [pd.Timestamp.now(tz='Asia/Seoul')]
            })
...
def create_dataset(features, window=10, strategy="단기", input_size=None):
    import pandas as _pd
    from config import MIN_FEATURES, get_class_groups as cfg_get_class_groups
    ...
    for c in price_cols:
        if c not in df.columns:
            ...
            df[c] = np.nan
    gains, labels_full, class_ranges = _make_labels(
