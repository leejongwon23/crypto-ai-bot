# predict.py — compressed + prediction-gate applied (no feature loss)
...
# ====== Gate (사진상의 요구: 학습 블록 끝났을 때만 예측 허용) ======
RUN_DIR="/persistent/run";os.makedirs(RUN_DIR,exist_ok=True)
PREDICT_GATE=os.path.join(RUN_DIR,"predict_gate.json")      # {"open":true, "opened_at": "...", "note":"group-x done"}
...
def is_predict_gate_open():
    try:
        ...
        o=json.load(open(PREDICT_GATE,"r",encoding="utf-8"));return bool(o.get("open",False))
    except Exception: return False
def open_predict_gate(note=""): json.dump({"open":True,"opened_at":_now_kst().isoformat(),"note":note},open(PREDICT_GATE,"w",encoding="utf-8"))
def close_predict_gate(): json.dump({"open":False,"closed_at":_now_kst().isoformat()},open(PREDICT_GATE,"w",encoding="utf-8"))
def _acquire_predict_lock():
    try:
        if os.path.exists(PREDICT_LOCK): return False
        open(PREDICT_LOCK,"w").write(_now_kst().isoformat());return True
    except Exception: return False
def _release_predict_lock():
    try:
        if os.path.exists(PREDICT_LOCK): os.remove(PREDICT_LOCK)
...
try:
    ...
    try: from window_optimizer import find_best_window
    except Exception: find_best_window=None
    def find_best_windows(symbol,strategy):
        try:
            if callable(find_best_window): best=int(find_best_window(symbol,strategy,window_list=[10,20,30,40,60],group_id=None))
            else: best=60
        except Exception: best=60
        return [best,best,best]
try:    from regime_detector import detect_regime
except:  detect_regime=lambda symbol,strategy,now=None:"unknown"
try:
...
try:
    import inspect; from model_io import load_model as _raw_load_model
    def load_model_any(path,model=None,**kwargs):
        try:
            ps=[p for p in inspect.signature(_raw_load_model).parameters.values() if p.kind in (p.POSITIONAL_ONLY,p.POSITIONAL_OR_KEYWORD)]
            if len(ps)<=1: return _raw_load_model(path)
            return _raw_load_model(path,model,**kwargs)
        except TypeError:
            try: return _raw_load_model(path,model,**kwargs)
            except Exception: return _raw_load_model(path)
        except Exception: return None
except Exception:
    def load_model_any(path,model=None,**kwargs):
        try:
            sd=torch.load(path,map_location="cpu")
            if isinstance(sd,dict) and model is not None: model.load_state_dict(sd);return model
            return sd
        except Exception: return None
...
from config import get_NUM_CLASSES,get_FEATURE_INPUT_SIZE,get_class_groups,get_class_return_range,class_to_expected_return

DEVICE=torch.device("cpu");MODEL_DIR="/persistent/models";PREDICTION_LOG_PATH="/persistent/prediction_log.csv"
NUM_CLASSES=get_NUM_CLASSES();FEATURE_INPUT_SIZE=get_FEATURE_INPUT_SIZE();now_kst=lambda:_now_kst()
MIN_RET_THRESHOLD=float(os.getenv("PREDICT_MIN_RETURN","0.01"))
EXP_STATE="/persistent/logs/meta_explore_state.json";EXP_EPS=float(os.getenv("EXPLORE_EPS_BASE","0.15"))
EXP_DEC_MIN=float(os.getenv("EXPLORE_DECAY_MIN","120"));EXP_NEAR=float(os.getenv("EXPLORE_NEAR_GAP","0.07"))
...
def _save_json(p,obj):
    try:
        os.makedirs(os.path.dirname(p),exist_ok=True);json.dump(obj,open(p,"w",encoding="utf-8"),ensure_ascii=False,indent=2)
    except Exception: pass
def _bump_use(symbol,strategy,model_path,explored=False):
    st=_load_json(EXP_STATE,{});key=f"{symbol}|{strategy}";st.setdefault(key,{})
    rec=st[key].setdefault(model_path,{"n":0,"n_explore":0,"last_explore_ts":0.0})
    rec["n"]+=1
    if explored: rec["n_explore"]+=1;rec["last_explore_ts"]=float(time.time())
...
def _feature_hash(row):
    try:
        ...
        r=[round(float(x),2) for x in arr];return hashlib.sha1(",".join(map(str,r)).encode()).hexdigest()
...
def _resolve_meta(weight_base):
    base=_stem(weight_base);cand=os.path.join(MODEL_DIR,f"{base}.meta.json")
    ...
    try:
        p=base.split("_"); 
        if len(p)>=3:
            sym,strat,mtype=p[0],p[1],p[2];cand2=os.path.join(MODEL_DIR,sym,strat,f"{mtype}.meta.json")
            if os.path.exists(cand2): return cand2
    except Exception: pass
    return None
def _glob_many(stem): out=[];[out.extend(glob.glob(f"{stem}{e}")) for e in _KNOWN_EXTS];return out

def get_available_models(symbol,strategy):
    try:
        if not os.path.isdir(MODEL_DIR): return []
        items=[];prefix=f"{symbol}_";needle=f"_{strategy}_"
        ...
        for g in _glob_many(os.path.join(MODEL_DIR,f"{symbol}_{strategy}_*group*cls*")):
            ...
            if mp and {"pt_file":gfn,"meta_path":mp} not in items: items.append({"pt_file":gfn,"meta_path":mp})
        items.sort(key=lambda x:x["pt_file"]);return items
...
def failed_result(symbol,strategy,model_type="unknown",reason="",source="일반",X_input=None):
    ...
    try:
        ...
        log_prediction(symbol=symbol,strategy=strategy,direction="예측실패",entry_price=0,target_price=0,model=str(model_type or "unknown"),
                       success=False,reason=reason,rate=0.0,timestamp=t,return_value=0.0,volatility=True,source=source,
                       predicted_class=-1,label=-1)
    except Exception as e: print(f"[failed_result log_prediction 오류] {e}")
    try:
        if X_input is not None: insert_failure_record(res,_feature_hash(X_input),feature_vector=np.array(X_input).flatten().tolist(),label=-1)
...
def predict(symbol,strategy,source="일반",model_type=None):
    ...
    try:
        ...
        if not symbol or not strategy: return failed_result(symbol or "None",strategy or "None",reason="invalid_symbol_strategy",X_input=None)
        ...
        if feat is None or feat.dropna().shape[0]<max(windows)+1: return failed_result(symbol,strategy,reason="feature_short",X_input=None)
        X=feat.drop(columns=["timestamp","strategy"],errors="ignore");X=MinMaxScaler().fit_transform(X)
        if X.shape[1]<FEATURE_INPUT_SIZE: X=np.pad(X,((0,0),(0,FEATURE_INPUT_SIZE-X.shape[1])),mode="constant")
        else: X=X[:,:FEATURE_INPUT_SIZE]
        ...
        rec_freq=get_recent_class_frequencies(strategy);feat_row=torch.tensor(X[-1],dtype=torch.float32)
        ...
        if _glob_many(os.path.join(MODEL_DIR,"evo_meta_learner")):
            try:
                ...
                if callable(predict_evo_meta):
                    ...
                    if cmin>=MIN_RET_THRESHOLD: final_cls=pred;meta_choice="evo_meta_learner"
        ...
        if final_cls is None:
            ...
            for i,m in enumerate(outs):
                ...
                filt=adj*mask
                if filt.sum()>0: filt=filt/filt.sum();pred=int(np.argmax(filt));p=float(filt[pred]);fused=True
                else: pred=int(np.argmax(adj));p=float(adj[pred]);fused=False
                score=p*(0.5+0.5*max(0.0,min(1.0,val_f1)))
                m.update({"adjusted_probs":adj,"filtered_probs":(filt if fused else None),"candidate_pred":pred,"success_score":score,"filtered_used":fused})
                scores.append((i,score,pred))
                if score>best_score: best_i, best_score, best_pred = i, score, pred; used_minret=fused
            ...
            if len(scores)>=2:
                ...
                if gap<=EXP_NEAR and random.random()<eps:
                    ...
                    if cands and cands[0][0]!=top1[0]: best_i=cands[0][0];best_pred=outs[best_i]["candidate_pred"];explore=True;meta_choice="best_single_explore"
            ...
            try:
                if meta_choice=="best_single_explore" or chosen.get("model_path"): _bump_use(symbol,strategy,chosen.get("model_path",""),explored=(meta_choice=="best_single_explore"))
            except Exception: pass
        ...
        try:
            ...
            if float(cmin_sel)<MIN_RET_THRESHOLD:
                ...
                if best_cls is None: return failed_result(symbol,strategy,reason="no_class_ge_min_return",X_input=X[-1])
                final_cls,chosen,used_minret=best_cls,best_m,True
        except Exception as e: print(f"[임계 가드 예외] {e}")

        # 로깅
        current=float(df.iloc[-1]["close"]);exp_ret=class_to_expected_return(final_cls,symbol,strategy);entry=current
        def _topk(p,k=3): return [int(i) for i in np.argsort(p)[::-1][:k]]
        topk=_topk((chosen or outs[0])["calib_probs"]) if (chosen or outs) else []
        note={"regime":regime,"meta_choice":meta_choice,
              ...
              "used_minret_filter":bool(used_minret),"explore_used":("best_single_explore" in str(meta_choice))}
        ensure_prediction_log_exists()
        log_prediction(symbol=symbol,strategy=strategy,direction="예측",entry_price=entry,target_price=entry*(1+exp_ret),
                       ...
                       predicted_class=final_cls,label=final_cls,note=json.dumps(note,ensure_ascii=False),top_k=topk,
                       success=False,reason="predicted",rate=exp_ret,return_value=0.0,
        ...
        try:
            for m in outs:
                ...
                if filt is not None and np.sum(filt)>0: pred_i=int(np.argmax(filt));src=filt
                else:
                    mask=np.zeros_like(adj,dtype=float)
                    for ci in range(len(adj)):
                        try:
                ...
                exp_i=class_to_expected_return(pred_i,symbol,strategy);top_i=[int(i) for i in np.argsort(src)[::-1][:3]]
        ...
        return {"symbol":symbol,"strategy":strategy,"model":"meta","class":final_cls,"expected_return":exp_ret,
                "timestamp":now_kst().isoformat(),"reason":("진화형 메타 최종 선택" if meta_choice=='evo_meta_learner' else f"최고 확률 단일 모델: {meta_choice}"),
...
def evaluate_predictions(get_price_fn):
    ...
    date_str=now_local().strftime("%Y-%m-%d");LOG_DIR="/persistent/logs";os.makedirs(LOG_DIR,exist_ok=True)
    EVAL=os.path.join(LOG_DIR,f"evaluation_{date_str}.csv");WRONG=os.path.join(LOG_DIR,f"wrong_{date_str}.csv")
    eval_h={"단기":4,"중기":24,"장기":168}
    try:
        with open(P,"r",encoding="utf-8-sig",newline="") as f_in:
            rd=csv.DictReader(f_in)
            if rd.fieldnames is None: print("[오류] prediction_log.csv 헤더 없음");return
            base=list(PREDICTION_HEADERS);extras=["status","return"];fields=base+[c for c in extras if c not in base]
            dir_name=os.path.dirname(P) or ".";fd,tmp=tempfile.mkstemp(prefix="predlog_",suffix=".csv",dir=dir_name,text=True);os.close(fd)
            with open(tmp,"w",encoding="utf-8-sig",newline="") as f_tmp,open(EVAL,"w",encoding="utf-8-sig",newline="") as f_eval,open(WRONG,"w",encoding="utf-8-sig",newline="") as f_wrong:
                ...
                for r in rd:
                    try:
                        if r.get("status") not in [None,"","pending","v_pending"]: w_all.writerow({k:r.get(k,"") for k in fields});continue
                        ...
                        pred_cls=to_int(r.get("predicted_class",-1),-1);label=to_int(r.get("label",-1),-1);r["label"]=label
                        ...
                        if dfp is None or "timestamp" not in dfp.columns:
                            ...
                            if not wrong_written:
                                wrong_writer=csv.DictWriter(f_wrong,fieldnames=sorted(r.keys()));wrong_writer.writeheader();wrong_written=True
                            wrong_writer.writerow({k:r.get(k,"") for k in r.keys()});continue
                        dfp=dfp.copy();dfp["timestamp"]=pd.to_datetime(dfp["timestamp"],errors="coerce").dt.tz_localize("UTC").dt.tz_convert("Asia/Seoul")
    ...
    except FileNotFoundError: print(f"[정보] {P} 없음 → 평가 스킵")
    except Exception as e:
...
def get_model_predictions(symbol,strategy,models,df,feat_scaled,window_list,recent_freq,regime="unknown"):
    ...
    for info in models:
        try:
            ...
            if seq.shape[0]<win: print(f"[⚠️ 데이터 부족] {symbol}-{strategy}-group{gid}");continue
            x=torch.tensor(seq,dtype=torch.float32).unsqueeze(0)
            model=get_model(mtype,input_size=inp_size,output_size=num_cls);model=load_model_any(model_path,model)
            if model is None: print(f"[⚠️ 모델 로딩 실패] {model_path}");continue
            model.eval()
            with torch.no_grad(): out=model(x.to(DEVICE));probs=F.softmax(out,dim=1).squeeze().cpu().numpy()
...
if __name__=="__main__":
    # 기본: 게이트가 열린 상태에서만 예측 동작
    # open_predict_gate("manual-run")  # 필요 시 주석 해제
    res=predict("BTCUSDT","단기");print(res)
    try:
        df=pd.read_csv(PREDICTION_LOG_PATH,encoding="utf-8-sig");print("[✅ prediction_log.csv 상위 20줄 출력]");print(df.head(20))
    except Exception as e: print(f"[오류] prediction_log.csv 로드 실패 → {e}")
