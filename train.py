...
def init(self, gamma: float = 2.0, weight: Optional[torch.Tensor] = None):
super().init()
...
def has_any_model_for_symbol(symbol: str) -> bool:
exts = (".ptz", ".safetensors", ".pt")
try:
if any(glob.glob(os.path.join(MODEL_DIR, f"{symbol}{e}")) for e in exts):
return True
...
def has_model_for(symbol: str, strategy: str) -> bool:
exts = (".ptz", ".safetensors", ".pt")
try:
if any(glob.glob(os.path.join(MODEL_DIR, f"{symbol}{strategy}_{e}")) for e in exts):
return True
...
def find_prev_model_for(symbol: str, prev_strategy: str) -> Optional[str]:
try:
candidates = []
for p in glob.glob(os.path.join(MODEL_DIR, f"{symbol}{prev_strategy}_.ptz")):
...
def scan_symbols_from_model_dir() -> List[str]:
syms = set()
try:
for p in glob.glob(os.path.join(MODEL_DIR, f"*_.")):
