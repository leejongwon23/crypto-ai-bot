...
def reset_all():
    ...
    try:
        def safe_clear_csv(file_path, fieldnames):
...
if __name__ == "__main__":
    ...
    sys.stdout.flush()
    start_scheduler()
    start_regfrom flask import Flask, jsonify, request, send_file
from recommend import main
import train
import os
import threading
...
if __name__ == "__main__":
    ...
    app.run(host="0.0.0.0", port=port)ular_prediction_loop()
    start_volatility_prediction_loop()
    send_message("[시스템 테스트] YOPO 서버가 정상적으로 실행되었으며 텔레그램 메시지도 전송됩니다.")
    print("✅ 테스트 메시지 전송 완료")
    sys.stdout.flush()
